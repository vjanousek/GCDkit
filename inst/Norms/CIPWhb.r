...
#####################################################################
#             CIPW norm  with hornblende and biotite                #
#                       (Hutchison, 1975)                           #
#####################################################################
# normsum - recast the norm to 100 %?
# cancrinite - is cancrinite present?
# spinel - shall be spinel calculated for rocks with SiO2<45 %?
CIPWhb<-function(wrdata,precision=getOption("gcd.digits"),normsum=FALSE,cancrinite=FALSE,spinel=FALSE,complete.results=FALSE){
    
CIPWhb.main<-function(wrdata,digits=getOption("gcd.digits"),normsum=FALSE,cancrinite=FALSE,spinel=FALSE){ 
result.names<-c("Q","C","Or","Ab","An","Lc","Ne","Kp","Nc","Ac","Ns","Ks","Di","MgDi","FeDi","Wo","Hy","En","Fs","Ol","Fo","Fa","Dcs","Mt","Il","Hm","Tn","Pf","Ru","Ap","Fr","Py","Cc","Sp","MgSp","FeSp","Bi","MgBi","FeBi","Hbl","Act","MgAct","FeAct","Ed","MgEd","FeEd","Ri","Sum") 
...
norm.names<-c("qtz","cor","ort","ab","an","lc","ne","kp","nc","ac","ns","ks","di","mgdi","fedi","wo","hy","en","fs","ol","fo","fa","cs","mt","il","hm","tn","pf","ru","ap","fr","pr","cc","sp","mgsp","fesp","bi","mgbi","febi","ho","act","mgact","feact","ed","mged","feed","ri","mgr","fer","femg")
...
if (y$fo + y$fa <= .5 * y$cor){
    ...
    if (y$cor >= 2 * d){
        ...
        y$mgsp <- y$mgsp + 2 * y$mgr * y$cor
        y$fesp <- y$fesp + 2 * y$fer * y$cor
        d <- d - .5 * y$cor
        y$fo <- y$fo - .5 * y$mgr * y$cor
        y$fa <- y$fa - .5 * y$fer * y$cor
        y$cor <- 0
    }
}
...
CIPWweight<-c(60.08,101.96,556.64,524.42,278.20,436.48,284.10,316.32,105.99,461.99,122.06,154.28,1,216.55,248.09,116.16,1,100.39,131.93,1,140.70,203.78,172.24,231.54,151.75,159.69,196.06,135.98,79.90,336.21,78.08,119.98,100.09,1,142.27,173.81,1,798.5,987.74,1,1,794.35,952.05,1,1632.48,1947.88,917.87)
...
w<-w*CIPWweight
    ...
    suma<-sum(w[-c(13,17,20,34,37,40,41,44)])
          
#REM Recast to 100% anhydrous?
if (normsum){
      w<-w*100/suma
          w[48]<-sum(w[-c(13,17,20,34,37,40,41,44)])
        }
    else{
          w[48]<-suma
    }
...
if(.Platform$GUI=="Rgui"){
    pb<-winProgressBar(title = "CIPW with hornblende and biotite", label = "Processing sample",min = 1, max = nrow(wrdata), initial = 1, width = 300)
}else{
    if(!getOption("gcd.shut.up"))pb<-txtProgressBar(title = "CIPW with hornblende and biotite", label = "Processing sample",min = 1, max = nrow(wrdata), initial = 1,char = "=",width = NA, style = 3)
}
...
results<-CIPWhb.main(wrdata,normsum=FALSE)
if(!complete.results){
    results<-results[,is.na(match(colnames(results),c("En","Fs","Fo","Fa","MgDi","FeDi","MgBi","FeBi","Act","FeAct","MgAct","Ed","FeEd","MgEd")))]
