...
NaAlK<-function(){
    ...
    temp1<-list(
        clssf=list("NULL",use=2:7,rcname=c("metaluminous/peraluminous","sodique","peralcalin","potassique","perpotassique","peralcalin","ultrapotassique")),
        ...
        GCDkit=list("NULL",plot.type="ternary",plot.position=13.5,plot.name="Diagramme Na2O-Al2O3-K2O moléculaire")
        )

    temp2<-list(
        text1=list("text",x=0.45,y=0.05,text="Sodique",col=plt.col[2]), 
        text2=list("text",x=0.58,y=0.05,text="Potassique",col=plt.col[2]),
        text3=list("text",x=0.85,y=0.05,text="Ultrapotassique",col=plt.col[2]),
        text4=list("text",x=0.0,y=0.5,text="Métalumineux\nPeralumineux",col=plt.col[2],adj=0),
        text5=list("text",x=0.0,y=0.4,text="Peralcalin",col=plt.col[2],adj=0), 
        text6=list("text",x=0.7,y=0.3,text="Perpotassique",col=plt.col[2],srt=60)
    )
...
sheet<<-list(demo=list(fun="plot",call=list(xlim=c(-.03,1.03),ylim=c(-0.08,1.03),main=annotate("Diagramme Na2O-Al2O3-K2O moléculaire"),bg="transparent",fg="black",asp=1,axes=FALSE,xlab="",ylab=""),template=temp))
