...
OConnorPlut<-function(ab=NULL,an=NULL,or=NULL){
    ...
    if (!just.sheets){
        ...
        if (z=="YES"){
        template<-list(
            clssf=list("NULL",use=2:6,rcname=c("trondhjémite","granite","quartz monzonite","granodiorite","tonalite")),
    ...
    temp<-list(        
        text1=list("text",x=15,y=sqrt(3)*10/2,text="Trondhjémite",col=plt.col[2],srt=30),
    ...
    common<-list(
        ...
        GCDkit=list("NULL",plot.type="ternary",plot.position=39,plot.name="Feldspath normatif (O'Connor 1965) ")
    )
