...
WinFloyd1<-function(){
template=list(
    clssf=list("NULL",use=2:12,rcname=c("Trachyandésite","Bas. alc.","Basanite/Nephelinite","Trachyte","Phonolite","Comendite/Pantellerite","Rhyolite","Rhyodacite/Dacite","Andésite","Andésite/Basalte","Basalte subalcalin")),
  ...
  if(getOption("gcd.plot.text")){
    template<-c(template,list(
        ...
        text4=list("text",x=0.02,y=0.008,text="Andésite/Basalte",col=plt.col[2],cex=1,adj=0),
        text5=list("text",x=0.02,y=.1,text="Rhyodacite/Dacite",col=plt.col[2],cex=1,adj=0),
        text6=list("text",x=0.04,y=.002,text="Basalte subalcalin",col=plt.col[2],cex=1,adj=0),
        text7=list("text",x=1,y=0.05,text="Trachy-\nandésite",col=plt.col[2],cex=1,adj=0),
        ...
        text11=list("text",x=.015,y=.04,text="Andésite",col=plt.col[2],cex=1,adj=0)))
}
...
#}
#}
sheet<<-list(demo=list(fun="plot",call=list(xlim=c(0.01,10),ylim=c(0.001,5),main=annotate("Diagramme Nb/Y - Zr/TiO2 (Winchester et Floyd 1977)"),bg="transparent",fg="black",asp=1,axes=TRUE,log="xy",xlab="Nb/Y",ylab=annotate("Zr/TiO2")),template=template))
}
