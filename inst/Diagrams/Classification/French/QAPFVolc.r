...
template<-list(
clssf=list("NULL",use=2:21,rcname=c("undefined","alk-fsp. rhyolite","rhyolite","dacite","trachyte quartzifere a fsp. alc.","quartz trachyte","quartz latite","basalte andésite","trachyte a fsp. alc","trachyte","latite","trachyte a fsp. alc et feldspathoides","foid-bearingtrachyte","latite a feldspathoides","phonolite","phonolite téphritique","téphrite/basanite phonolitique","feldsphathoidite phonolitique","feldsphathoidite téphritique","feldsphathoidite")),
...
uppertext<-list(    
text3=list("text",x=22,y=35,text="rhyolite a fsp. alc.",col=plt.col[2],srt=60),
text4=list("text",x=42,y=35,text="rhyolite",col=plt.col[2]),
text6=list("text",x=70,y=35,text="dacite",col=plt.col[2]),
text8=list("text",x=10,y=11,text="trachyte quartzifere\na fsp. alc.",col=plt.col[2],adj=1),
text9=list("text",x=27,y=11,text="trachyte\nquartzifere",col=plt.col[2]),
text10=list("text",x=50,y=11,text="latite\nquartzifere",col=plt.col[2]),
text11=list("text",x=77,y=9,text="basalte\nandésite",col=plt.col[2]),
...
text18=list("text",x=0,y=80,text="1 - trachyte a fsp. alc",col=plt.col[2],adj=0)
)
...
text23=list("text",x=50,y=-4,text="latite a feldspathoides",col=plt.col[2],adj=0.5),
...
text28=list("text",x=65,y=-20,text="téphrite/basanite\nphonolitique",col=plt.col[2],adj=0.5),
text29=list("text",x=79,y=-30,text="téphrite/basanite",col=plt.col[2],adj=0.5,srt=63),
text30=list("text",x=49,y=-60,text="feldsphathoidite\nphonolitique",col=plt.col[2],adj=1),
text30=list("text",x=51,y=-60,text="feldsphathoidite\ntéphritique",col=plt.col[2],adj=0),
text1=list("text",x=50,y=-80,text="feldsphathoidite",col=plt.col[2]),
text34=list("text",x=0,y=-70,text="4 - trachyte a fsp. Alc\net feldspathoides",col=plt.col[2],adj=0)
)
if (triangle=="auto"){
...
if (getOption("gcd.plot.text")){
    if (triangle=="upper") {template<-c(template,upper,uppertext);ylim<-c(-3,103);main<-"Diagramme QAPF - Roches sursaturées en silice"}
    if (triangle=="lower") {template<-c(template,lower,lowertext);ylim<-c(-103,3);main<-"Diagramme QAPF - Roches sous-saturées en silice"}
    if (triangle=="both") {template<-c(template,upper,lower);ylim<-c(-103,103);main<-"QAPF diagram"}
}else{
    if (triangle=="upper") {template<-c(template,upper);ylim<-c(-3,103);main<-"Diagramme QAPF - Roches sursaturées en silice"}
    if (triangle=="lower") {template<-c(template,lower);ylim<-c(-103,3);main<-"Diagramme QAPF - Roches sous-saturées en silice"}
    if (triangle=="both") {template<-c(template,upper,lower) ;ylim<-c(-103,103);main<-"QAPF diagram"}   
}
