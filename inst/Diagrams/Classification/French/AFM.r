...
temp1<-list(
        clssf=list("NULL",use=2:3,rcname=c("série calco-alcaline","série tholéitique")),
...
temp2<-list(
        text4=list("text",x=0.5,y=0.55,text="Série tholéitique",col=plt.col[2]),
        text5=list("text",x=0.5,y=0.1,text="Série calco-alcaline",col=plt.col[2])
        )
...
sheet<<-list(demo=list(fun="plot",call=list(xlim=c(-.03,1.03),ylim=c(-0.05,1.03),main=annotate("Diagramme AFM (Irvine et Baragar 1971)"),xlab="",ylab="",bg="transparent",fg="black",asp=1,axes=FALSE),template=temp))
