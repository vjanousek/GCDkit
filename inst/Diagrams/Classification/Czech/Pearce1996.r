...
y.data<<-WR[,"Zr"]/WR[,"Ti"]
temp<-list(
        clssf=list("NULL",use=2:11,rcname=c("bazalt","alkalický bazalt","foidit","andezit, bazaltický andezit","trachyandezit","tefrifonolit","ryolit, dacit","trachyt","fonolit","alkalický ryolit")),
        ...
        GCDkit=list("NULL",plot.type="binary",plot.position=27,plot.name="Nb/Y - Zr/Ti diagram (modifikovaný Pearcem 1996)")
)

temp1<-list(
        text1=list("text",x=0.08,y=0.003,text="bazalt",col=plt.col[2],adj=0.5),
        text2=list("text",x=1.5,y=0.01,text="alkalický\nbazalt",col=plt.col[2],adj=0.5),
        text3=list("text",x=8,y=0.01,text="foidit",col=plt.col[2],adj=c(0,0.5)),
        text4=list("text",x=0.1,y=0.03,text="andezit\nbazaltický andezit",col=plt.col[2],adj=0.5,srt=17),
        text5=list("text",x=1.5,y=0.06,text="trachy-\nandezit",col=plt.col[2],adj=0.5,srt=17),
        text6=list("text",x=10,y=0.095,text="tefrifonolit",col=plt.col[2],adj=0.5,srt=17),
        text7=list("text",x=0.1,y=0.2,text="ryolit\ndacit",col=plt.col[2],adj=0.5),
        text8=list("text",x=1.5,y=0.2,text="trachyt",col=plt.col[2],adj=0.5),
        text9=list("text",x=10,y=0.4,text="fonolit",col=plt.col[2],adj=0.5),
        text10=list("text",x=0.8,y=0.6,text="alkalický\nryolit",col=plt.col[2],adj=0.5)    
    )

if(getOption("gcd.plot.text"))temp<-c(temp,temp1)


sheet<<-list(demo=list(fun="plot",call=list(xlim=c(0.01,100),ylim=c(0.001,2),main=annotate("Nb/Y - Zr/Ti diagram (modifikovaný Pearcem 1996)"),col="green",bg="transparent",fg="black",xlab=annotate("Nb/Y"),ylab=annotate("Zr/Ti"),log="xy"),template=temp))
}
