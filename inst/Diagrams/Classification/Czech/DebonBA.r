...
x.data<<-debon[,"B"]
y.data<<-debon[,"A"]
sheet<<-list(demo=list(fun="plot", call=list(xlim=c(0,500),ylim=c(-500,500),col="green",bg="transparent",fg="black",main=annotate("Debon and Le Fort B-A"),xlab="B = Fe + Mg +Ti",ylab="A = Al - (K + Na + 2Ca)"),template=temp))
ee<-DebonCalc(milli)
return(ee)
}
