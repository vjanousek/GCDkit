...
OConnorPlut<-function(ab=NULL,an=NULL,or=NULL){
    ...
    if (!just.sheets){
        ...
        if (z=="YES"){
        template<-list(
            clssf=list("NULL",use=2:6,rcname=c("trondhjemit","granit","kvarc monzonit","granodiorit","tonalit")),
        ...
        template<-list(
            clssf=list("NULL",use=2:6,rcname=c("Trondhjemit","Granit","Quartz monzonite","Granodiorit","Tonalit")),
    ...
    temp<-list(        
        text1=list("text",x=15,y=sqrt(3)*10/2,text="Trondhjemit",col=plt.col[2],srt=30),
        text2=list("text",x=40+7/2,y=sqrt(3)*7/2,text="Granit",col=plt.col[2]),
        text3=list("text",x=42+25/2,y=sqrt(3)*25/2,text="Kvarc\nmonzonit",col=plt.col[2],srt=60),
        text4=list("text",x=27+30/2,y=sqrt(3)*30/2,text="Grano-\ndiorite",col=plt.col[2]),
        text5=list("text",x=5+40/2,y=sqrt(3)*40/2,text="Tonalit",col=plt.col[2],srt=60)
    )
    ...
    common<-list(
        ...
        GCDkit=list("NULL",plot.type="ternary",plot.position=39,plot.name="Živcový ternární diagram (O'Connor 1965) ")
    )
