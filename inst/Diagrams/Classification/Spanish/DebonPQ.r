...
x.data<<-debon[,"P"]
y.data<<-debon[,"Q"]
sheet<<-list(demo=list(fun="plot", call=list(xlim=c(-400,120),ylim=c(-20,270),col="green",bg="transparent",fg="black",main=annotate("Diagrama P-Q (Debon y Le Fort)"),xlab="P = K - (Na + Ca)",ylab="Q = Si/3-(K+Na+2Ca/3)"),template=temp))
ee<-DebonCalc(milli)
return(ee)
}
