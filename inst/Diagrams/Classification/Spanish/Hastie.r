...
temp<-list(
        #classification
        clssf=list("NULL",use=2:4,rcname=c("serie toleítica ","serie calcoalcalina","high-K calc-alkaline and shoshonite series")),
...
temp1<-list(
        text1=list("text",x=68,y=0.1,text="Serie\nToleítica ",col=plt.col[2],adj=c(0,0.5)),
        text2=list("text",x=68,y=1,text="Serie\nCalcoalcalina",col=plt.col[2],adj=c(0,0.5)),
        text3=list("text",x=68,y=60,text="Serie Calcoalcalina-K\ny Shoshonítica",col=plt.col[2],adj=c(0,0.5)),
...
sheet<<-list(demo=list(fun="plot",call=list(xlim=c(70,0),ylim=c(0.01,100),main=annotate("Diagrama Co-Th (Hastie et al. 2007)"),col="green",bg="transparent",fg="black",xlab="Co",ylab="Th", log="y"),template=temp))
}
