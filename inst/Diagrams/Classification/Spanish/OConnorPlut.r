...
OConnorPlut<-function(ab=NULL,an=NULL,or=NULL){
    ...
    if (!just.sheets){
        ...
        if (z=="YES"){
        template<-list(
            clssf=list("NULL",use=2:6,rcname=c("trondhjemita","granito","cuarzo monzonita","granodiorita","tonalita")),
        ...
        template<-list(
            clssf=list("NULL",use=2:6,rcname=c("Trondhjemita","Granito","Quartz monzonite","Granodiorita","Tonalita")),
    ...
    temp<-list(        
        text1=list("text",x=15,y=sqrt(3)*10/2,text="Trondhjemita",col=plt.col[2],srt=30),
        text2=list("text",x=40+7/2,y=sqrt(3)*7/2,text="Granito",col=plt.col[2]),
        text3=list("text",x=42+25/2,y=sqrt(3)*25/2,text="Cuarzo\nmonzonita",col=plt.col[2],srt=60),
        text4=list("text",x=27+30/2,y=sqrt(3)*30/2,text="Grano-\ndiorite",col=plt.col[2]),
        text5=list("text",x=5+40/2,y=sqrt(3)*40/2,text="Tonalita",col=plt.col[2],srt=60)
    )
    ...
    common<-list(
        ...
        GCDkit=list("NULL",plot.type="ternary",plot.position=39,plot.name="Feldespato normativo (O'Connor 1965) ")
    )
