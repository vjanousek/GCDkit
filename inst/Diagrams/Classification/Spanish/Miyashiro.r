...
temp<-list(
        clssf=list("NULL",use=2:3,rcname=c("serie calcoalcalina","serie toleítica")),
...
temp2<-list(
        text1=list("text",x=77,y=0.7,text="Serie calco-\nalcalina",col=plt.col[2],adj=c(1,0.5)),
        text2=list("text",x=47,y=4,text="Serie Toleítica",col=plt.col[2],adj=c(0,0.5))
        )
...
sheet<<-list(demo=list(fun="plot",call=list(xlim=c(45,78),ylim=c(0,6),main=annotate("Diagrama SiO2-FeOt/MgO (Miyashiro 1974)"),col="green",bg="transparent",fg="black",xlab=annotate("SiO2"),ylab=annotate("FeOt/MgO")),template=temp))
}
