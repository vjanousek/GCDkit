...
temp2<-list(
        text4=list("text",x=0.5,y=0.55,text="Serie Toleítica",col=plt.col[2]),
        text5=list("text",x=0.5,y=0.1,text="Serie Calcoalcalina",col=plt.col[2])
        )
...
sheet<<-list(demo=list(fun="plot",call=list(xlim=c(-.03,1.03),ylim=c(-0.05,1.03),main=annotate("Diagrama AFM (Irvine y Baragar 1971)"),xlab="",ylab="",bg="transparent",fg="black",asp=1,axes=FALSE),template=temp))
